...
async function CekJawabanUjianUserToDB(data) {
  const client = await pool.connect();
  try {
    let jawaban = false;

    const { id_soal, id_jawaban, id_mengambil_ujian } = data;
    ...
    const queryTextSave = `
      INSERT INTO public.jawabanujianuser(
        ujian_diambil, soal_ujian, jawaban_user, status_jawaban)
      VALUES ($1, $2, $3, $4)
    `;
    const queryValuesSave = [id_mengambil_ujian, id_soal, id_jawaban, jawaban];
...
module.exports = { CekJawabanUserToDB,CekJawabanUjianUserToDB };
